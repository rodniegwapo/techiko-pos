...
const columns = [
  { title: "Product", dataIndex: "product", key: "product", align: "left", width: "40%" },
  { title: "Stock", dataIndex: "stock", key: "stock", align: "center", width: "15%" },
...
const dataSource = computed(() => {
  return props.inventories?.data?.map(inventory => ({
    key: inventory.id,
    id: inventory.id,
  ...
  >
    ...
    <template #bodyCell="{ column, record }">
      <template v-if="column.key === 'product'">
        <div class="flex items-center space-x-3">
          ...
          <div class="w-10 h-10 bg-gray-200 rounded-lg flex items-center justify-center">
            <img 
              v-if="record.product?.representation_type === 'image' && record.product?.representation"
              :src="record.product.representation"
              :alt="record.product.name"
              class="w-full h-full object-cover rounded-lg"
            />
            <div 
              v-else-if="record.product?.representation_type === 'color' && record.product?.representation"
          ...
          <!-- Product Info -->
          <div>
            <p class="font-semibold text-gray-900">{{ record.product?.name || 'Unknown Product' }}</p>
            <p class="text-sm text-gray-500">{{ record.product?.category?.name || 'No Category' }}</p>
          </div>
        </div>
      </template>
      ...
      <template v-else-if="column.key === 'stock'">
        <div class="text-center">
          ...
          <p class="text-xs text-gray-500">{{ record.product?.unit_of_measure || 'pcs' }}</p>
        </div>
      </template>
      ...
      <template v-else-if="column.key === 'status'">
        <a-tag class="w-fit" :color="getStockStatusColor(record.status)">
          <component :is="getStockStatusIcon(record.status)" :size="16" class="mr-1" />
    ...
    <template #emptyText>
      <div class="text-center py-8">
        ...
        <p class="text-sm text-gray-400">Try adjusting your filters or add some inventory</p>
      </div>
    </template>
  </a-table>
</template>
